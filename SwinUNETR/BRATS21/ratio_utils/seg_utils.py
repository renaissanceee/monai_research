...
def gather_files(folder_pred, folder_ref):
    files_pred = subfiles(join(folder_pred,'seg'), suffix='.nii.gz', join=False)
    files_prob = subfiles(join(folder_pred,'prob'), suffix=".npz", join=False)  # for probs
    files_ref = subfiles(join(folder_pred, 'gt'), suffix='.nii.gz', join=False)
    # files_ref = subfiles(folder_ref, suffix='.nii.gz', join=False)
    
    files_ref = [join(folder_pred,'gt', i) for i in files_pred]
    files_pred = [join(folder_pred,'seg', i) for i in files_pred]
    files_prob = [join(folder_pred,'prob', i) for i in files_prob]
...
def region_or_label_to_mask_prob_add(segmentation: np.ndarray,
    ...
    if np.isscalar(region_or_label):
        ...
        for r in region_or_label:  # 1,2,4
...
def region_or_label_to_mask_prob_max(segmentation: np.ndarray,
    ...
    if np.isscalar(region_or_label):
        ...
        mask = np.zeros(segmentation.shape[1:])
        for r in region_or_label:  # 1,2,4
            mask = np.maximum(mask, segmentation[r, :, :, :])
    return mask, np.sum(mask)
